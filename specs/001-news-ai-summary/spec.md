...
8. **Given** 用户点击"Export Markdown"，**Then** 弹出文件保存对话框，用户可选择路径并指定文件名，系统保存 Markdown 文件
...
### User Story 10 - 增强摘要功能 (Priority: P1)

用户获取更全面、更详细的新闻总结，包含更多重要新闻、标的信息和详细内容。

**Why this priority**: 解决当前实现的局限性，提供更优质的总结服务。

**Independent Test**: 运行 GUI，选择 feeds 生成总结，检查生成的 Markdown 是否包含 8-12 条重要新闻、标的一览和详细摘要内容。

**Acceptance Scenarios**:

1. **Given** 用户生成新闻总结，**When** AI 分析新闻，**Then** 系统提取 8-12 条重要新闻（而非原来的 3-5 条）
2. **Given** 用户生成财经新闻总结，**When** 新闻中包含股票信息，**Then** 系统识别并提取标的信息（股票代码、公司名称）作为投资参考
3. **Given** 用户生成新闻总结，**When** 部分 feeds 的文章内容较短（如韭研公社），**Then** 用户可选择是否为这些 feeds 抓取文章链接获取完整内容后再分析
4. **Given** 用户生成新闻总结，**Then** 每条摘要更详细（150-300 字），包含更多背景和分析
5. **Given** 用户生成新闻总结，**When** 有同主题新闻，**Then** 系统对同主题新闻进行合并，避免信息重复
6. **Given** 用户查看生成的报告，**Then** 报告包含"标的一览"部分，列出所有识别出的标的信息
7. **Given** 用户在桌面应用中点击"生成总结"，**Then** 系统显示每选中的 Feed 旁边有"补充链接信息"复选框（默认勾选），用户可单独控制每个 Feed 是否从链接抓取完整内容
...
- **FR-021**: 系统 MUST 提取 8-12 条重要新闻（而非原来的 3-5 条）进行详细总结
- **FR-022**: 系统 MUST 识别并提取财经新闻中的标的信息（股票代码、公司名称），并在报告中提供标的一览
- **FR-023**: 系统 MUST 支持为指定 Feed 抓取链接获取完整内容后再分析
- **FR-024**: 系统 MUST 提供更详细的摘要内容（150-300 字/条），并对同主题新闻进行合并
- **FR-025**: 桌面应用 MUST 支持用户通过复选框单独控制每个 Feed 是否从链接抓取完整内容（默认勾选）
...
- **Entry**: 代表一条新闻条目，包含 id、title、url、published_at、summary、content、feed_id 属性
- **SummarizeResult**: 代表 AI 总结结果，包含 summary、highlights、keywords、sentiment、tickers 属性
- **Ticker**: 代表股票/投资标的信息，包含 code、name 属性
...
- **扩展新闻数量**: 提取 8-12 条重要新闻（原来 3-5 条），提供更全面覆盖
- **标的信息提取**: 识别并提取财经新闻中的股票代码、公司名称作为投资参考
- **链接内容获取**: 支持为指定 Feed 抓取链接获取完整内容后再分析，桌面应用提供复选框让用户控制
- **详细摘要内容**: 每条摘要要求 150-300 字，包含更多背景和分析
- **同主题合并**: 对同主题新闻进行合并，避免信息重复
- **标的一览**: 在 Markdown 报告中新增"标的一览"部分
